...
/**
 * Status badge component with premium color system
 * Design System: Status colors - Pending: Sunset, In Progress: Ocean, Resolved: Emerald
 */
export function StatusBadge({ status, priority, sla, escalationLevel }) {
    ...
    if (escalationLevel && escalationLevel > 1) {
        badges.push(
            <span key="escalation" className="px-3 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
                ...
                {images.length > maxDisplay && (
                    <div className="h-20 w-20 flex-shrink-0 bg-gray-100 rounded-lg border border-gray-200 flex items-center justify-center">
                        <span className="text-xs text-gray-500 font-medium">+{images.length - maxDisplay}</span>
                    </div>
                )}
            </div>
        </div>
