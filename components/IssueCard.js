...
function IssueContent({ issue, isExpanded, onToggleExpanded }) {
    return (
        <>
            <p className="text-gray-600 text-sm leading-relaxed mb-4 px-0">
...
export default function IssueCard({ issue, onStatusChange, userRole, showSensitiveData = false }) {
    ...
    if (!issue || !issue._id) {
        return <div className="p-4 text-center text-gray-500">Invalid issue data</div>;
    }
            ...
            <div className="flex items-center gap-3">
                {user && (
                    <button
                        ...
                        disabled={isUpvoting}
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-white text-gray-700 rounded-full border border-gray-200 hover:bg-gray-50 transition-colors disabled:opacity-50"
                    >
    ...
    return (
        <SpotlightCard className="w-full rounded-2xl border border-gray-100 bg-white shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
            ...
            <div className="sm:hidden p-5 flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 gap-3 pt-2">
                     <Link
                        href={`/issues/${issue._id}`}
                        className="w-full min-h-[48px] flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow-sm transition-colors text-sm"
                    >
                        View Details
                    </Link>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                         {user && (
                            <button
                                onClick={handleUpvote}
                                disabled={isUpvoting}
                                className="w-full min-h-[48px] flex items-center justify-center gap-2 bg-white border border-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-50 disabled:opacity-50 text-sm"
                            >
                        ...
                        {(userRole === 'admin' || userRole === 'municipal' || userRole === 'department') && (
                            <div className="relative">
                                <select
                                    className="w-full min-h-[48px] appearance-none bg-white border border-gray-200 text-gray-700 font-medium rounded-lg px-4 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
                                    value={issue.status}
                                    onChange={(e) => handleStatusChange(e.target.value)}
                                >
                                    <option value="pending">Pending</option>
                ...
                <div className="p-6">
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-start gap-4">
                            <div className="flex-shrink-0 w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-xl">
